...
function fieldStatusBadges(f,connected){
 const blocked=f.blockedUntil>=s.round,scorched=f.scorchedUntil>=s.round,open=f.garrison.filter(c=>c.open).length,statuses=[];
 if(f.owner!==null&&!blocked&&!connected)statuses.push(['cut','⛓',t('game.node_status.cut')]);
 if(blocked)statuses.push(['blocked','⊘',t('game.node_status.blocked',{round:f.blockedUntil})]);
 if(scorched)statuses.push(['scorched','♨',t('game.node_status.scorched',{round:f.scorchedUntil})]);
 if(f.owner!==null&&!blocked&&!scorched&&open===BALANCE.campaign.garrisonUpkeep.idleAtOpen)statuses.push(['idle','Ⅱ',t('game.node_status.idle',{n:open})]);
 if(f.owner!==null&&!blocked&&!scorched&&open>=BALANCE.campaign.garrisonUpkeep.negativeAtOpen)statuses.push(['negative','−',t('game.node_status.negative',{n:open})]);
 if(!statuses.length)return '';
 return '<span class="node-statuses">'+statuses.map(([kind,icon,tip])=>'<span class="node-status-marker status-'+kind+'" data-status="'+kind+'" data-tooltip="'+esc(tip)+'" title="'+esc(tip)+'" aria-label="'+esc(tip)+'">'+icon+'</span>').join('')+'</span>';
}
...
function mapView(){
 ...
 return (targeting?'<div class="targeting-note">'+t('game.targeting',{name:strategyText(targeting).name})+btn(t('game.cancel'),'cancel-target','text-button')+'</div>':'')+'<div class="war-map"><div class="map-title"><div class="eyebrow">'+t('game.map.eyebrow')+'</div><h2>'+mapText(map.id).name+'</h2></div><div class="map-stage '+(s.fields.length>9?'dense-map':'')+'"><div class="map-camera" style="--map-x:'+mapViewport.x+'px;--map-y:'+mapViewport.y+'px;--map-scale:'+mapViewport.scale+'"><div id="visual-mount" class="scene-mount"></div>'+geoBackdrop(map)+topology+'<div class="node-layer">'+s.fields.map(site=>{const connected=site.owner===null||cutLine[site.owner].has(site.id);return '<button class="map-node owner-'+(site.owner===null?'null':sideOf(site.owner))+' '+(!connected?'supply-cut ':'')+(site.id===focus?'focused ':'')+(selectedLinks.has(site.id)?'route-neighbor ':'')+(reachable(s,p,site)&&site.owner!==p?'reachable':'')+'" data-action="focus" data-id="'+site.id+'" data-map-x="'+site.x+'" data-map-y="'+site.y+'" style="left:'+site.x+'%;top:'+site.y+'%"><span class="node-icon">'+fieldIcon(site)+'</span>'+fieldStatusBadges(site,connected)+(focusHasSea&&(site.id===f.id||isSeaLink(s,f.id,site.id))?'<span class="sea-marker" title="'+t('game.sea_marker')+'">'+seaMarkerSVG+'</span>':'')+'<b>'+fieldName(s.opt.map,site)+'</b>'+mapGarrison(site,p)+'<small>'+(site.owner===null?t('game.node_neutral'):site.owner===p?t('game.node_own'):t('game.node_enemy'))+' · '+t('game.node_capacity',{n:garrisonLimit(site)})+'</small></button>'}).join('')+'</div></div><div class="map-controls"><button data-map-control="in" aria-label="'+t('game.mapctrl_in')+'">＋</button><button data-map-control="out" aria-label="'+t('game.mapctrl_out')+'">－</button><button data-map-control="reset" aria-label="'+t('game.mapctrl_reset')+'">⌖</button></div><span class="map-gesture-hint">'+t('game.mapctrl_hint')+'</span><span class="map-compass">N<br>↑</span></div>'+
...
function handTray(){
 ...
 const excess=Math.max(0,pl.hand.length-BALANCE.campaign.safeHandSize),upkeep=excess*BALANCE.campaign.excessHandUpkeep;
 const logistics=s.opt.rules==='campaign'?'<span class="hand-logistics '+(excess?'warning':'')+'" title="'+esc(t('game.hand.logistics_title',{safe:BALANCE.campaign.safeHandSize,cost:upkeep}))+'">'+t('game.hand.logistics',{safe:BALANCE.campaign.safeHandSize,cost:upkeep})+'</span>'+btn(t('game.hand.demobilize'),'open-demobilize','hand-demobilize',s.phase!=='campaign'||s.active!==p||isAI()||!pl.hand.length):'';
...
function reserveView(config){
 const p=s.players[config.player],sort=config.sort||'rank',order=cards=>[...cards].sort(sort==='suit'?(a,b)=>a.suit-b.suit||a.rank-b.rank||a.id-b.id:(a,b)=>a.rank-b.rank||a.suit-b.suit||a.id-b.id),cards=order(p.reserve),demobilized=order(p.demobilized||[]);
 return '<div class="reserve-heading"><div><h2>'+t('modal.reserve.title',{name:p.name})+'</h2><p>'+t('modal.reserve.desc')+'</p></div><span class="hand-sort-toggle" role="group" aria-label="'+t('modal.reserve.sort_label')+'"><button class="'+(sort==='rank'?'active':'')+'" data-action="reserve-sort" data-id="rank" aria-pressed="'+(sort==='rank')+'">'+t('modal.reserve.sort_rank')+'</button><button class="'+(sort==='suit'?'active':'')+'" data-action="reserve-sort" data-id="suit" aria-pressed="'+(sort==='suit')+'">'+t('modal.reserve.sort_suit')+'</button></span></div><div class="reserve-cards">'+(cards.map(c=>card(c,{small:true})).join('')||'<p>'+t('modal.reserve.empty')+'</p>')+'</div><section class="demobilized-pile"><h3>'+t('modal.demobilized.title',{n:demobilized.length})+'</h3><p>'+t('modal.demobilized.desc')+'</p><div class="reserve-cards">'+(demobilized.map(c=>card(c,{small:true})).join('')||'<p>'+t('modal.demobilized.empty')+'</p>')+'</div></section>'+garrisonRoster(config.player)+btn(t('modal.close'),'close','primary');
}
function demobilizeView(config){
 const p=viewer(),pl=s.players[p],sort=config.sort||'rank',chosen=config.chosen||new Set(),cards=sortHand(pl.hand,sort),remaining=pl.hand.length-chosen.size,excess=Math.max(0,remaining-BALANCE.campaign.safeHandSize),cost=excess*BALANCE.campaign.excessHandUpkeep;
 return '<div class="eyebrow">'+t('modal.demobilize.eyebrow')+'</div><h2>'+t('modal.demobilize.title')+'</h2><p class="modal-desc">'+t('modal.demobilize.desc',{safe:BALANCE.campaign.safeHandSize})+'</p><div class="reserve-heading"><span>'+t('modal.demobilize.selected',{n:chosen.size,remaining,cost})+'</span><span class="hand-sort-toggle" role="group" aria-label="'+t('game.hand.sort_label')+'"><button class="'+(sort==='rank'?'active':'')+'" data-action="demobilize-sort" data-id="rank" aria-pressed="'+(sort==='rank')+'">'+t('game.hand.sort_rank')+'</button><button class="'+(sort==='suit'?'active':'')+'" data-action="demobilize-sort" data-id="suit" aria-pressed="'+(sort==='suit')+'">'+t('game.hand.sort_suit')+'</button></span></div><div class="demobilize-cards">'+cards.map(c=>card(c,{small:true,interactive:true,selected:chosen.has(c.id),action:'demobilize-toggle'})).join('')+'</div><div class="modal-actions">'+btn(t('modal.cancel'),'close','secondary')+btn(t('modal.demobilize.confirm',{n:chosen.size}),'demobilize-submit','primary danger',!chosen.size)+'</div>';
}
...
function modalView(kind=modal){
 ...
 else if(kind&&kind.kind==='demobilize')content=demobilizeView(kind);
...
document.addEventListener('click',e=>{
 ...
 if(a==='open-demobilize'){showModal({kind:'demobilize',chosen:new Set(),sort:handSort});return}
 if(a==='demobilize-sort'&&modal?.kind==='demobilize'){modal.sort=id==='suit'?'suit':'rank';render();return}
 if(a==='demobilize-toggle'&&modal?.kind==='demobilize'){const cid=Number(id);modal.chosen.has(cid)?modal.chosen.delete(cid):modal.chosen.add(cid);render();return}
 if(a==='demobilize-submit'&&modal?.kind==='demobilize'){const ids=[...modal.chosen];modal=null;resume();perform({type:'demobilize',ids});return}
