...
function header(menu=false){
 ...
 '<div class="top-actions">'+(menu?'<button class="identity-chip" data-action="edit-identity" aria-label="选择名字和徽记"><span class="identity-emblem"><i>'+identityLogo+'</i></span><span class="identity-copy"><small>指挥官</small><b>'+esc(identityName)+'</b></span><i aria-hidden="true">›</i><span class="identity-hint" role="tooltip">选择名字和徽记</span></button>':'')+btn(muted?'音效关闭':'音效开启','sound','text-button')+btn('规则','rules','text-button')+(menu?'':btn('暂停','pause','icon-button'))+'</div></header>';
}
