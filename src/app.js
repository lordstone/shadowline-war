...
function mapView(){
 ...
 '<div class="target-actions">'+(s.actionSpent?'<span class="action-reminder" title="'+t('game.action.reminder_title')+'">'+t('game.action.reminder_text')+'</span>':'')+(f?.owner===null?btn(t('game.action.occupy')+' →','occupy','primary '+(primaryAvailable&&!placementValid?'needs-selection':''),!primaryAvailable||!can,'title="'+(placementValid?t('game.action.occupy_title_ok'):t('game.action.occupy_title_need'))+'" data-selection-ready="'+placementValid+'"'):f?.owner===1-p?btn(t('game.action.attack')+' →','attack','primary danger',!primaryAvailable||!can)+btn(t('game.action.siege'),'siege','secondary',!primaryAvailable||!can||f.garrison.length<=3||s.players[p].supply<3,'title="'+t('game.action.siege_title')+'"'):f?.owner===p?btn(t('game.action.reorganize')+' →','reorganize','primary '+(primaryAvailable&&!placementValid?'needs-selection':''),!primaryAvailable,'title="'+(placementValid?t('game.action.reorganize_title_ok'):t('game.action.reorganize_title_need'))+'" data-selection-ready="'+placementValid+'"')+btn(t('game.action.rotate'),'open-rotation','secondary',!actionable||!f.garrison.length||!s.players[p].hand.length||s.rapidRedeployUsed||s.players[p].supply<1,'title="'+t('game.action.rotate_title')+'"'):'')+
 btn(t('game.action.supply'),'supply','secondary',!actionable||s.supplyUsed||s.players[p].supply<2||!s.deck.length,'title="'+t('game.action.supply_title')+'"')+btn(t('game.action.pass'),'pass','secondary pass-action',!actionable,'title="'+t('game.action.pass_title')+'"')+'</div></div></div>';
}
function line(p){
...
function battleView(){
 ...
 (s.phase==='tactics'?btn(t('game.battle.continue')+' →','continue','primary',!active):s.phase==='counter'?btn(reveals.size?t('game.battle.reveal_confirm',{n:reveals.size})+' →':t('game.battle.reveal_first'),'reveal','primary',!active||reveals.size<1||reveals.size>2):btn(t('game.battle.deploy')+' →','deploy','primary',!active||!valid))+
...
document.addEventListener('pointerdown',e=>{
 if(e.target.closest('#visual-mount')||!e.target.closest('.map-stage'))return;
 if(e.target.closest('[data-map-control]')||e.target.closest('.map-node')||e.target.closest('.target-bar'))return;
 const id=e.pointerId;
 mapPointers.set(id,{x:e.clientX,y:e.clientY});
 if(mapPointers.size===1){mapDrag={x:e.clientX,y:e.clientY,vx:mapViewport.x,vy:mapViewport.y,moved:false}}
 else if(mapPointers.size===2){const [a,b]=[...mapPointers.values()];mapDrag={pinch:true,dist:Math.hypot(a.x-b.x,a.y-b.y),scale:mapViewport.scale}}
});
...
const endPointer=e=>{mapPointers.delete(e.pointerId);if(mapPointers.size<2&&mapDrag?.pinch)mapDrag=null;if(mapPointers.size===0)mapDrag=null};
