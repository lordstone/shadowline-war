...
export async function browserChecks(browser,url='http://127.0.0.1:4173/'){
 ...
 await load(s);const adjacent=s.fields.find(f=>f.owner===null&&f.links.some(id=>s.fields.find(x=>x.id===id)?.owner===0));
 ...
 checks.push('short Chrome viewport matrix keeps top HUD, target actions, twelve-card hand, map controls and full-width tactic dock separated and visible');
