...
export async function browserChecks(browser,url='http://127.0.0.1:4173/'){
 ...
 assert.equal(await page.locator('.top-actions [data-action="tutorial"]').count(),1);await page.locator('[data-action="tutorial"]').click();assert.equal(await page.locator('.tutorial-progress i').count(),7);
 for(let step=0;step<7;step++){await page.locator('.tutorial-shot img').waitFor();await page.waitForFunction(()=>{const img=document.querySelector('.tutorial-shot img');return img?.complete&&img.naturalWidth>500});assert.match(await page.locator('.tutorial-head>span').innerText(),new RegExp('^'+(step+1)+' / 7$'));const box=await page.locator('.modal').evaluate(e=>{const r=e.getBoundingClientRect();return {top:r.top,bottom:r.bottom,height:r.height,viewport:innerHeight,scrollHeight:e.scrollHeight}});assert.ok(box.top>=0&&box.bottom<=box.viewport&&box.height<=box.viewport,JSON.stringify(box));await page.locator('[data-action="tutorial-step"]').last().click()}
 assert.equal(await page.locator('.modal-overlay').count(),0);checks.push('seven-step illustrated tutorial loads every real UI screenshot, advances correctly and fits a 390px iPhone viewport');
