...
export async function browserChecks(browser,url='http://127.0.0.1:4173/'){
 ...
 s=createGame({strategies:false,mode:'ai',map:'western_front',seed:1414});const paris=s.fields.find(f=>f.id==='paris');while(paris.garrison.length<5)paris.garrison.push({...s.players[1].hand.pop(),open:paris.garrison.length<3});s.raid=true;s=act(s,0,{type:'attack',field:paris.id}).state;validate(s);await load(s);
