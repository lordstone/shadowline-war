...
export async function strategyMarketCheck(browser,url='http://127.0.0.1:4173/'){
 ...
 saved=await page.evaluate(()=>JSON.parse(localStorage.getItem('shadowline-war-v1')));assert.equal(saved.strategyLocked[0].length,0);assert.equal(saved.active,0);assert.deepEqual(errors,[]);await page.close();return 'visible target actions at app-window width; public market purchase, hover tooltip, refill, supply cost and next-own-turn unlock';
}
